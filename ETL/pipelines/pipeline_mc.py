...
from etl_sdt.configs.config_mc import *
...
def process_file(file_path, collection, preview=False, preview_n=5, timeline=False):
    ...
    """
    df = open_excel(file_path=file_path)
    logger.info(f"File {file_path} loaded successfully.")

    df = df[1:]
    chunks = chunking(df, num_chunks=50, chunking_variable_id='Pat ID')
    ...
    for _, chunk in enumerate(chunks):
        ...
        df_chunk = fe.process_mc()

        df_chunk = fe.post_process_mc()
...
def main(file_path, preview=False, dry_run=False, preview_n=5, delete_collection=False, timeline=False):
    ...
    process_file(file_path, collection, preview=preview, preview_n=preview_n, timeline=timeline)
