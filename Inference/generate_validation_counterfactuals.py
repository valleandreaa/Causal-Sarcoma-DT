...
def main():
    ...
    # This is the full list of validation patient IDs.
    val_patient_ids_all = [
        1019562, 7549036, 11087934, 156590, 11182976, 807705, 7047993, 11235569, "608927ksw",
        "1031579ksw", 11214100, 100783061, 643779, 254801, 8530105, 1081132, 8243730, 7306607,
        1938649, 2078700, 429237, "1041910ksw", "1043154ksw", 788512, 630105, 378937, 547718,
        7512785, 8046982, 724985, 11135940, 11209564, 3387453, 11054891, 7008643, 11059470,
        1096743, 8400173, "1072741ksw", 1076998, "1159023ksw", 1103470, 737027, 7241196, 3132164,
        7546042, 1051298, 1187592, 11010372, 1049321, 432862, 1108596, 3787370, 1189147,
        "1094152ksw", 1067388, 399447, 1197312, 1060591, 8574708, 1117034, 1861972, 7271801,
        1001523, "453800ksw", 8358676, 1009734, 11105013, 468387, 1106794, 1046388, 1242676,
        1367536, 974226, 1264843, 7210343, "1029742ksw", 200683833, 599774, "1084900ksw",
        511225, 1138511, 1067138, 376056, 457007, 1035025, 11032261, 1257289, 11140299,
        11169073, 1150095, 8523532, 501398, 1858971, 679704, 8371682, 7250508, 365946,
        1102534, 11096881, 1229129, 1252042, 576627, 1864637, 550712, "1130473ksw", 1214822,
        1055539, 802149, "1051612ksw", 1219380, "1101660ksw", 809812, 217482, 1128802,
        11070462, 7153203, 1061876, 4760794, "1124750ksw", 11264743, 1039395, 8608321,
        298338, 11093547, 8034078, 1068383, 1059922, 554253, 322863, 323244, 5383358,
        11078569, 527406, 8484374, "1065132ksw", 610110, 729747, 1062817, 1075614, 1124790,
        294030, 633675, 11191523
    ]

    # This is the list of patient IDs that were used for the main generation task.
    # We will find the intersection to only process validation patients that are available.
    patient_ids_available = [
        156590, 215635, 242834, 323244, 345717, 526227, 527406, 541208,
    ...
    patient_ids_to_process = list(set(val_patient_ids_all) & set(patient_ids_available))
    
    print(f"Found {len(patient_ids_to_process)} validation patients to process out of {len(val_patient_ids_all)} total validation IDs.")

    output_file = "counterfactual_scenarios_validation_patients.csv"
    
    # Process the filtered list of validation patients
    process_multiple_patients(patient_ids_to_process, meta_file, config_file, output_file)
